...
- **Cost:** cite **mean** seconds on caption-32. Pseudo **304 s**, Always **348 s**, Rolling **45 s**. The gate is 13% cheaper than Always and still ~6.8× Rolling.
...
**Wanted:** about as good as Rolling, much cheaper than Always (348 s). Stretch: near Rolling (45 s). Ours = controller on frozen Self Forcing, not a host swap.

128 quality: tail tie, Dyn% + IQ win, subject loss. Cost still ~6.8× Rolling. Remaining paper is **cheapen** (search-early or prune k) or **Rolling window-exit** (`2026-08-30_wan_rf_intervene.md`). Not another seed search. Neighbors: Video-T1 (prune), TANGO critic without LoRA, TTC as the AdaSteer / Prefix-match exhibit. Do not build an agent.
