...
| V2V prefix-continuation (real video history → 30 s AR) | Closer match to the claim than T2V. Condition on 1–2 s of real video (Panda / UCF / Kinetics), generate 30 s, VBench on the full clip. | **IN FLIGHT 2026-08-20.** Smoke job **16069897** (`v2v_panda_smoke` NOTTA n=2). Probe job **16069898** (`v2v_panda_probe` knob_probe n=2). Cluster HEAD `a8d3d7d`. | Do **not** submit N=8 until both complete. Spec: [`2026-08-20_wan_v2v_sampling_bakeoff_spec.md`](paper_tables/2026-08-20_wan_v2v_sampling_bakeoff_spec.md). |
| `v2v_panda_bakeoff_8v` | Sampling-space bake-off beyond gating: notta, seed_bon, motion_bon, shift_search (if probe moves), backtrack. | **NOT SUBMITTED.** Waiting on 16069897 + 16069898. | `SKIP_SHIFT=1` only if probe `shift_live=false`. |
