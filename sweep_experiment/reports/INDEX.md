...
| 7. Phase 0 gating feature pipeline (`scripts/sbatch/submit_per_video_feature_pipeline.sh` — Stage 1a `extract_video_features_for_tta.py`, 1b `compute_diffusion_ood_score.py`, 1c `compute_tier3_probes.py`, Stage 2 `correlate_tta_gain_with_features.py`) | 2026-06-14 (re-fire ready after **round-3** bug-fix commit on `main`) | Round-1 fix (`0802d5a`): dtype+contiguity coercion. Round-2 fix (`557292c`): defensive shape+container pipeline with asserts. BOTH FAILED on job 10739993 — the round-2 helper completes without asserting (frames are correctly (T,H,W,3) uint8 contiguous) yet `cv2.calcHist` at the immediately-following line still rejected the per-frame slice. Diagnosed as an OpenCV-4.9.0 Python-binding incompatibility we cannot beat by tweaking inputs. Round-3 fix (THIS commit on `main`): **bypass cv2 entirely** — `cv2.calcHist` / `cv2.normalize` / `cv2.compareHist` replaced with numpy equivalents (`np.histogramdd` + manual L2/L1 normalisation + analytic Bhattacharyya) in both `count_cuts_histogram` and `rgb_histogram_entropy_mean`. The round-2 helper `_to_uint8_hwc_stack_for_cv2` is preserved for shape coercion. Output is bit-equivalent on well-formed inputs (sanity-tested on macOS: identical-frame distance = 0.000000, random-pair distance ≈ 0.019, histogram pixel-sum == H·W). See [ANALYSIS_LOG 2026-06-14 — Phase 0 round-3 fix](ANALYSIS_LOG.md). New job IDs TBD on cluster restart. | TBD on re-fire (pre-bug-fix wall budget was ~3-6 GPU-h Stage 1a + ~6-12 GPU-h Stage 1c + ~30 min Stage 2; re-fire should match) | On the cluster, after `git pull`: **delete or rename the existing header-only CSVs first** (`rm sweep_experiment/reports/per_video_analysis/2026-06-09/{video_features,diffusion_ood_scores,tier3_probe_features}.csv` — the wrappers default to `RESUME=0` so they will overwrite cleanly anyway, but explicitly removing the broken outputs is safer because partial-row CSVs from a failed run could otherwise be picked up by the correlation Stage 2 join), then `bash scripts/sbatch/submit_per_video_feature_pipeline.sh`. The round-3 numpy bypass has no remaining cv2-binding failure mode for the histogram features; if Stage 1a errors again it will be on a different code path (`laplacian_variance_mean`, CLIP/DINO, or PyAV decode) and the traceback will surface it cleanly. |
