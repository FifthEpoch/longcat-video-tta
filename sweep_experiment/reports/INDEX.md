...
**Owners:** Wenchen (PI) and any active agent. Last updated: 2026-06-14.
...
| 7. Phase 0 gating feature pipeline — **extract-only re-fire** (`scripts/sbatch/submit_per_video_feature_pipeline.sh`; Stage 1a only this wave) | 2026-06-14 (~04:00 AM UTC+8) | Extract `10756670`; correlate `10756671` (depends on extract only). Submitted at git HEAD **`5d2a871`** (round-3 numpy cv2 bypass). Submit cmd: `SKIP_OOD=1 SKIP_TIER3=1 bash scripts/sbatch/submit_per_video_feature_pipeline.sh`. Broken `video_features.csv` deleted pre-submit; prior-run OOD + Tier3 CSVs (~1000 rows each) retained. Round-1/2 fixes failed on jobs 10737160 / 10739993; round-3 fix in `5d2a871`. See [ANALYSIS_LOG 2026-06-14 cluster-ops](ANALYSIS_LOG.md). | ~3-6 GPU-h Stage 1a; correlate ~30 min after extract | After extract completes: `wc -l sweep_experiment/reports/per_video_analysis/2026-06-09/video_features.csv` (expect ~1001). Correlate `10756671` will be **features-only partial** (OOD/Tier3 skipped). After extract succeeds, re-run **full** correlate using existing OOD + Tier3 CSVs: `bash scripts/sbatch/submit_per_video_feature_pipeline.sh` (no SKIP flags). |
| 8. Mod 2 smoke re-fire (`sweep_experiment/sbatch/submit_smoke_vae_decoder_tta.sh`) | 2026-06-14 (~04:00 AM UTC+8) | **`10756668`** (submitted; status: in flight). Cluster HEAD at submit: **`5d2a871`** — **missing OOM fix `b802835`**. Prior invalid smoke `10737006` (100/100 OOM). | ~3-5 GPU h | **Action required:** `git pull` (must reach `b802835`); if `10756668` still PD → `scancel 10756668` and re-fire. Verify checkout before trusting results. See [ANALYSIS_LOG 2026-06-14 cluster-ops](ANALYSIS_LOG.md). |
