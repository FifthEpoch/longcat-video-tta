...
| 8. Mod 2 smoke re-fire (`sweep_experiment/sbatch/submit_smoke_vae_decoder_tta.sh`) | 2026-06-14 | **`10756668` COMPLETED — invalid (100/100 OOM; ran with `b802835`, offload insufficient).** Round-2 two-phase load fix pushed to `main`. | ~3-5 GPU h | **Re-fire:** `git pull` then `bash sweep_experiment/sbatch/submit_smoke_vae_decoder_tta.sh`. Verify `.out` shows `[mem] after VAE-only load` ≈ 8–15 GiB (not ~137 GiB). See [ANALYSIS_LOG 2026-06-14 round-2](ANALYSIS_LOG.md). |
