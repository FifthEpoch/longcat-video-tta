...
| Wan non-weight next methods | Motion verifier, `{shift,cfg}` probe, prefix backtrack now hosted on V2V (not T2V 128). CachedSearch / sink-ckpt / HG-f stay wave 2. | Wave 1 implemented in `run_v2v_chunked.py`. | **No TTC.** Picker insight: [`2026-08-21_wan_v2v_picker_insight.md`](paper_tables/2026-08-21_wan_v2v_picker_insight.md) — motion is a vote in the two-sided sum, not a constraint. Resimulate band-constraint pick on N=8 cand logs before another generate. |
