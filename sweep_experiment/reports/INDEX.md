...
**Owners:** Wenchen (PI) and any active agent. Last updated: 2026-08-24.
...
| `v2v_panda_confirm_32v` | notta vs seed_bon only. N=32. Same V2V protocol. | **DONE 2026-08-21.** Generate + VBench 16122823. | **seed_bon FAIL as a Dyn method.** Tail −8.8% (12/32). VBench Dyn **0/0**. Subject **+0.039**, IQ −0.77. Identity damper. Also: prompts were filename stems (`panda 0013`), not scene captions. [`stem prompt`](paper_tables/2026-08-24_wan_v2v_panda_stem_prompt.md). Table: [`2026-08-21_wan_v2v_overnight_vbench.md`](paper_tables/2026-08-21_wan_v2v_overnight_vbench.md). |
...
| `v2v_panda_rf_always_32v` | `rf_always_search` k=4 on RF rolling. Same pick as rf_sick/rf_pseudo, no gate. **N=32**. | **GENERATE DONE 2026-08-24.** **16288114** COMPLETED 0:0 53m, mp4=32/32. VBench **16288115** waits on 113. | Host twin. Cite vs `rolling_notta`. Do not call until VBench + SF twin. |
| `v2v_panda_caption_32v` | Caption replay WAVE=1: notta, rolling, SF/RF family, both always-search. **N=32**. | **IN FLIGHT 2026-08-24 18:44.** **16310318** notta R 11m gh125; **16310319** rolling R gh126. Rest PD QOSMaxGRESPerUser. VBench **16310330**. | 2-way H200 cap. No WAVE=2 until sidecar is `metadata_csv`. |
| `v2v_panda_caption_closed_32v` | WAVE=2: seed/quiet/live/appear + host-split. **N=32**. | **READY after WAVE=1.** | Closed methods, caption-conditioned. |
| `v2v_panda_caption_8v` | WAVE=3: remaining N=8 discovery. | **READY after WAVE=1.** | No shift_search / knob_probe. |
| `v2v_panda_caption_128v` | WAVE=4: caption notta + rolling. **N=128**. | **READY after WAVE=1 harvest.** | Hosts only. Do not mix with stem rolling-128. |
| `v2v_panda_adasteer_8v` | AdaSteer on SF V2V: `ada_fixed`, `ada_stream`, `ada_resid`. Captions. **N=8**. | **SUBMIT-READY 2026-08-24.** `submit_v2v_adasteer.sh`. | Confirm δ updates on Wan AR. Cite vs caption notta. Spec: [`adasteer`](paper_tables/2026-08-24_wan_v2v_adasteer_spec.md). |
