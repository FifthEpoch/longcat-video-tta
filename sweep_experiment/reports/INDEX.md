...
| 7. Phase 0 gating feature pipeline (`scripts/sbatch/submit_per_video_feature_pipeline.sh` — Stage 1a `extract_video_features_for_tta.py`, 1b `compute_diffusion_ood_score.py`, 1c `compute_tier3_probes.py`, Stage 2 `correlate_tta_gain_with_features.py`) | 2026-06-13 (re-fire ready after **round-2** bug-fix commit on `main`) | Round-1 fix (`0802d5a`, 2026-06-13 AM): coerced cv2.calcHist input dtype+contiguity. Re-fired as job 10737160 → still `errored: 1000` with the *same* `cv2.calcHist` "wrong type" error, now at line 362 (post-helper). Round-2 fix (THIS commit on `main`): replaces the helper with a defensive shape+container coercion pipeline (torch tensor → ndarray, list → stack, CHW → HWC, float → uint8) plus runtime asserts; applied at BOTH `cv2.calcHist` call sites (`count_cuts_histogram` AND `rgb_histogram_entropy_mean`). See [ANALYSIS_LOG 2026-06-13 — Phase 0 round-2 fix](ANALYSIS_LOG.md). New job IDs TBD on cluster restart. | TBD on re-fire (pre-bug-fix wall budget was ~3-6 GPU-h Stage 1a + ~6-12 GPU-h Stage 1c + ~30 min Stage 2; re-fire should match) | On the cluster, after `git pull`: **delete or rename the existing header-only CSVs first** (`rm sweep_experiment/reports/per_video_analysis/2026-06-09/{video_features,diffusion_ood_scores,tier3_probe_features}.csv` — the wrappers default to `RESUME=0` so they will overwrite cleanly anyway, but explicitly removing the broken outputs is safer because partial-row CSVs from a failed run could otherwise be picked up by the correlation Stage 2 join), then `bash scripts/sbatch/submit_per_video_feature_pipeline.sh`. If the helper's `assert` ever fires on a real video, the `.err` log will print the offending `arr.shape` and channel-axis size verbatim (no more cryptic OpenCV `Sequence item has wrong type` line). |
