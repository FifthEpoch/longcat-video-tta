...
| V2V prefix-continuation (real video history → 30 s AR) | Closer match to the claim than T2V. Condition on 1–2 s of real video (Panda / UCF / Kinetics), generate 30 s, VBench on the full clip. | **SUBMIT-READY 2026-08-20.** Runner `run_v2v_chunked.py`. | Smoke / probe / N=8 bake-off. Spec: [`2026-08-20_wan_v2v_sampling_bakeoff_spec.md`](paper_tables/2026-08-20_wan_v2v_sampling_bakeoff_spec.md). I2V-still scale-up stays closed. |
| `v2v_panda_bakeoff_8v` | Sampling-space bake-off beyond gating: notta, seed_bon, motion_bon, shift_search (if probe moves), backtrack. | **SUBMIT-READY.** `SMOKE=1` then `PROBE=1` then full submit. | Do not scale past N=8 until tail motion beats notta without IQ/subject collapse. Analyzer: `analyze_v2v_bakeoff.py`. |
