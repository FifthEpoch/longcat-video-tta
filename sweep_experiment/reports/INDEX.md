...
| `v2v_panda_sf_always_32v` | `sf_always_search` k=4. Same pick as pseudo, no prefix gate. **N=32**. | **READY 2026-08-24.** Wave: `submit_v2v_always_search_wave.sh`. | Splits gate vs pick. k=4 = family + CachedSearch BoN-4. [`k`](paper_tables/2026-08-24_wan_v2v_always_search_k.md). |
| `v2v_panda_rf_always_32v` | `rf_always_search` k=4 on RF rolling. Same pick as rf_sick/rf_pseudo, no gate. **N=32**. | **READY 2026-08-24.** Same wave as SF always, or `submit_v2v_rf_always_search.sh` if SF already queued. | Host twin. Cite vs `rolling_notta`, not vs SF. |
