...
| 7. Phase 0 gating feature pipeline — **extract-only re-fire** (`scripts/sbatch/submit_per_video_feature_pipeline.sh`; Stage 1a only this wave) | 2026-06-14 | Extract `10756670` **FAILED** (header-only CSV at `5d2a871`). **Round-4 fix pushed to `main`** — eliminates remaining cv2 (`laplacian_variance_mean`, `decode_window` resize). Re-fire after `git pull`. Submit: `SKIP_OOD=1 SKIP_TIER3=1 bash scripts/sbatch/submit_per_video_feature_pipeline.sh`. Smoke: `MAX_VIDEOS=3 sbatch scripts/sbatch/run_extract_video_features.sbatch`. See [ANALYSIS_LOG round-4](ANALYSIS_LOG.md). | ~1-3 GPU-h Stage 1a (PSD skipped by default) | `wc -l sweep_experiment/reports/per_video_analysis/2026-06-09/video_features.csv` (expect ~1001). Correlate `10756671` was features-only partial — re-run full correlate after extract succeeds. |
| 8. Mod 2 smoke re-fire (`sweep_experiment/sbatch/submit_smoke_vae_decoder_tta.sh`) | 2026-06-14 | **`10756668` COMPLETED — invalid (100/100 OOM; `b802835`).** **`10780689` COMPLETED — invalid (100/100 OOM; `6be47d2` two-phase load insufficient).** Round-3 strict two-phase TTA→gen fix pushed to `main`. | ~3-5 GPU h | **Re-fire:** `git pull` then `bash sweep_experiment/sbatch/submit_smoke_vae_decoder_tta.sh`. Verify `.out` shows `PHASE 1` + `[mem] end of phase-1` < 25 GiB, then `PHASE 2: generation (lazy DiT load)`. See [ANALYSIS_LOG round-3](ANALYSIS_LOG.md). |
