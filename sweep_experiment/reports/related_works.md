...
**Status:** Decision log + Markdown archive of the Related Works draft. As of May 23, 2026, the **canonical Related Works prose now lives in LaTeX** at `paper/sections/related_work.tex` (Overleaf-ready, compiles with `paper/main.tex` and `paper/refs.bib`). The Markdown draft below is preserved verbatim as the lock-in record so the conversion to LaTeX is auditable; any new edits should go to the `.tex` file, not this Markdown.

Locked decisions: Proposal C (hybrid narrative-aligned), dual framing (steering-first, positioned as a modulation-pathway TTA subtype) in Section 2.2, long-context Panda treated as an honest caveat in Section 2.1 and Section 2.2 (option A), no ControlNet/CFG paragraphs, author-year citation style via `natbib` `\citep{}` with BibTeX keys in `paper/refs.bib`. The three organizational proposals (A: method-axis, B: problem-axis, C: hybrid) are kept below for archive.
...
These models all share two architectural features that AdaSteer's design depends on: (i) a single global timestep embedding pathway, and (ii) per-block adaLN modulation layers that re-project that pathway into shift, scale, and gate vectors. Despite this shared structure, none of these works expose the modulation pathway as a *test-time* adaptation interface; all adaptation is left to fine-tuning, LoRA, or full retraining. This is the gap AdaSteer fills. We focus on the *standard-horizon* video continuation regime where the conditioning frames provide a usable adaptation signal for the entire generation window; pushing AdaSteer to long-horizon generation, where the adaptation signal becomes a smaller fraction of the predicted future, is an honest open problem that we revisit in Section~5.

#### 2.2 Test-Time Steering via the Modulation Pathway: A Modulation-Pathway TTA Subtype

We position AdaSteer as a *test-time steering* method: it does not update any pretrained weights and does not add any trainable layers to the network. It only learns a 512-dimensional residual that is added to the timestep embedding before each block's frozen adaLN projection consumes it. Conceptually, this is a strict subtype of test-time adaptation (TTA) that targets only the modulation pathway, and we place it in that broader literature for grounding.
...
Two features of this literature are important for AdaSteer. First, the consistent finding that *very small, structured* adaptation surfaces (BN statistics, channel-wise affine parameters, a handful of feature-wise biases) often perform as well as or better than freeing larger parts of the network. AdaSteer extends this finding from discriminative models to a 13.6B-parameter video DiT, but in a more constrained way: instead of unfreezing a few parameters across many layers, we add a single shared residual upstream of all block-wise modulation projections. We refer to this design as a *modulation-pathway TTA subtype* because it inherits the TTA framing (one update per test instance, no source-data access at adaptation time) while restricting the adaptation surface to a single conditioning interface.

Second, almost all prior TTA work is built on top of discriminative classifiers or segmentation models; very little extends to large generative models, and even less to video. The TTA-for-generation entries we are aware of cluster around customization (Section 2.3) and test-time *search* over noise trajectories ([Liu et al., 2025 — Video-T1](https://arxiv.org/abs/2503.18942)), which is orthogonal to weight or embedding-residual adaptation. AdaSteer is, to our knowledge, the first method to expose the modulation pathway of a billion-scale video DiT as a test-time steering interface for single-clip continuation. The empirical scope of this paper is the standard-horizon continuation regime; the question of whether the same modulation-pathway interface can be made horizon-robust is an open research direction we discuss in Section~5.
...
## Locked Decisions (May 23, 2026)

1. **Organization.** Proposal C (six-subsection hybrid narrative-aligned).
2. **Framing.** Dual framing in Section 2.2: lead with *steering*, then position the method as a *modulation-pathway TTA subtype* so we inherit the broader TTA literature without overclaiming. This is applied above.
3. **Long-context Panda.** Option A (caveat). The corrected 999-video long-context Panda run shows FVD 278.7 → 284.1 (+5.4) for AdaSteer, with pointwise PSNR/SSIM/LPIPS/FID essentially within noise. Treated honestly as a caveat in Sections 2.1 and 2.2; the related-work draft does not centerpiece this regression. The PI flagged this as a target for follow-up experimentation (see `experiment_tracker/next_actions.md`), not a negative-result framing for the paper.
4. **Extra paragraphs.** Skipped ControlNet/T2I-Adapter and CFG/autoguidance per PI direction.
5. **Citation style.** Author-year via `\citep{}`; BibTeX entries are in `sweep_experiment/reports/refs.bib`.
