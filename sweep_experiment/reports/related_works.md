...
**Status:** Locked draft (May 23, 2026). Uses Proposal C (hybrid narrative-aligned). Locked decisions: dual framing (steering-first, positioned as a modulation-pathway TTA subtype) in Section 2.2; long-context Panda treated as an honest caveat in Section 2.1 and Section 2.2 (option A); no ControlNet/CFG paragraphs; author-year citation style with BibTeX keys in `refs.bib`. The three organizational proposals are kept below for archive.
