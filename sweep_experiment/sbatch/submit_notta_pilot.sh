#!/bin/bash
# ============================================================================
# Re-generate the NO-TTA baseline for the 200v OOD budget PILOT *in-series*.
#
# WHY: the pilot's original vs-NO-TTA comparison borrowed NOTTA from a DIFFERENT
# series (panda_1000v_standard/NOTTA). The generation seed is
# `base_seed(42) + position-within-chunk`, so a video sitting at a different
# position in a different pool got a DIFFERENT seed -> different initial noise ->
# the per-video "oracle Δ vs NO-TTA" was confounded (verified 2026-07-21 with
# scripts/verify_seed_alignment.py: 1/200 = 0.5% seed match).
#
# This runs NO-TTA on the SAME pool (panda_ood_budget_pilot_480p) with the SAME
# 2×100 / SEED=42 chunking the 12 pilot configs used, so each video is seeded
# identically to its config arms. NO-TTA == METHOD=full, num_steps=0, lr=0.
#
# Lands as sibling arm:  panda_ood_budget_pilot/NOTTA/chunk_*
#
# Submit:
#   cd /scratch/wc3013/longcat-video-tta
#   bash sweep_experiment/sbatch/submit_notta_pilot.sh
# Dry-run:
#   DRY_RUN=1 bash sweep_experiment/sbatch/submit_notta_pilot.sh
#
# AFTER completion, confirm alignment:
#   python3 scripts/verify_seed_alignment.py \
#       --run-a sweep_experiment/results/panda_ood_budget_pilot/S10_LR5e3 \
#       --run-b sweep_experiment/results/panda_ood_budget_pilot/NOTTA
#   # expect ~100% match
# ============================================================================
set -euo pipefail
...
# SAME dataset + results tree + chunking as submit_adasteer_budget_pilot.sh.
...
# MUST match the pilot config chunking (2 × 100) so per-video seeds line up.
...
# Geometry — identical to submit_adasteer_budget_pilot.sh.
...
DRY_RUN="${DRY_RUN:-0}"
# Save mp4s by default (needed for VBench + reusable videos, and so NOTTA
# matches the grid arms). Set NO_SAVE_VIDEOS=1 for metrics-only.
...
# Dataset guard — never launch against an incomplete pool (misaligned per-video sets).
...
echo "============================================================"
echo "NO-TTA baseline PILOT re-generation (seed-matched, in-series)"
echo "============================================================"
...
echo "  seed         : 42 (matches pilot configs)"
...
echo ""
echo "Submitted ${count} NOTTA pilot jobs -> ${RESULTS_SUBDIR}/${RUN_ID}"
echo ""
echo "After completion, CONFIRM seed alignment:"
echo "  python3 scripts/verify_seed_alignment.py \\"
echo "      --run-a ${RESULTS_SUBDIR}/S10_LR5e3 \\"
echo "      --run-b ${RESULTS_SUBDIR}/NOTTA"
echo "  # expect ~100% match; then re-merge + re-run the pilot analysis/charts."
echo "============================================================"
