#!/bin/bash
# Submit 200-video anchor-regularized AdaSteer objective sweeps.
#
# Defaults to DRY_RUN=1 so it is safe to run locally or on the cluster.
# To submit for real:
#   DRY_RUN=0 bash sweep_experiment/sbatch/submit_anchor_reg_200.sh

set -euo pipefail

ACCOUNT="${ACCOUNT:-torch_pr_36_mren}"
DRY_RUN="${DRY_RUN:-1}"
TIME_STANDARD="${TIME_STANDARD:-12:00:00}"
...
run_sweep sweep_experiment/configs/panda_200_anchor_reg.yaml \
  "${PROJECT_ROOT}/datasets/panda_200_480p"
run_sweep sweep_experiment/configs/ucf101_200_anchor_reg.yaml \
