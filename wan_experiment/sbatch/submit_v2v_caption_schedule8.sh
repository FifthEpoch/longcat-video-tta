#!/bin/bash
# Caption N=8 non-linear Rolling Forcing timestep list.
# Existing student. Same T as the live denoising_step_list.
# Host = caption-32 Rolling (do not remake). Do not remake cite-128.
#
...
SERIES="${SERIES:-v2v_panda_caption_schedule_8v}"
N_VIDEOS="${N_VIDEOS:-8}"
if [[ "${SMOKE:-0}" == "1" ]]; then
    N_VIDEOS=2
...
echo "---- caption schedule8 preflight (first-segment, not stem) ----"
python3 - <<'PY'
...
submit_method rolling_linger
submit_method rolling_dump
...
echo "Caption schedule8 N=${N_VIDEOS}. Cite vs caption Rolling first-8, not leftover ρ."
echo "First sidecar must print rf_step_list native=... used=... and prompt_source=metadata_csv."
...
echo "    --baseline-dir ${PROJECT_ROOT}/wan_experiment/results/v2v_panda_caption_32v \\"
echo "    --series-dir ${ROOT}"
echo "Cite vs Rolling host, not analyzer-vs-Self-Forcing. No TTC. No I2V. Do not remake cite-128."
