#!/bin/bash
# Resume sf_rewind after 16266878 (exit 2, 8/32). Skip-existing keeps
# the 8 mp4s. VBench afterok on this rewind + the three still-running
# generate jobs (or already-completed 0:0).
#
#   cd /scratch/wc3013/longcat-video-tta && git pull --ff-only origin main
#   bash wan_experiment/sbatch/submit_v2v_sf_rewind_resume.sh
...
# The three generate jobs from the first wave. afterok is a no-op if
# they already completed 0:0.
...
J=$(sbatch --parsable --account="${ACCOUNT}" --time="${GEN_WALL}" \
    --export=ALL,METHOD=sf_rewind,SEARCH_K=1,${COMMON} \
    "${SB}/run_v2v_chunked.sbatch")
echo "V2V ${SERIES} sf_rewind resume n=${N_VIDEOS} job ${J}"
...
VB=$(sbatch --parsable --account="${ACCOUNT}" --time="${VBENCH_WALL}" \
    --dependency="afterok:${J}:${SIBLINGS}" \
    --export=ALL,SERIES_DIR="${ROOT}",VIDEO_DIRS="${VIDEO_DIRS}",CLIPS=full \
    "${SB}/run_i2v_vbench.sbatch")
echo "VBench full-clip job ${VB} afterok ${J}:${SIBLINGS}"
echo "Skip-existing keeps the 8 rewind mp4s. Leave 16266879–881 running."
