#!/bin/bash
# Four cheap host hypotheses. No LoRA. No TTC. Do not scale from N=8.
#
...
#
# Baselines: bake-off SF notta AND lineage rolling_notta (same 8).
# Promote past 8 only if tail > both relevant baselines and IQ/subject hold.
#
...
SERIES="${SERIES:-v2v_panda_host_split_8v}"
N_VIDEOS="${N_VIDEOS:-8}"
...
echo "H1/H4 N=8. 2-way H200: queues behind 128 VBench if still running."
...
echo "    --series-dir ${ROOT}"
echo "H1 vs SF notta AND vs lineage rolling. H4 vs its own host."
echo "Do not scale from this table. Cancel this wave only:  scancel ${JOBS[*]}"
