#!/bin/bash
# Caption N=8 slide-the-guessed-picture after pass 1 (pred-only).
# Ordinary white extras. Not leftover-flow HIWYN extras.
...
SERIES="${SERIES:-v2v_panda_caption_pwarp_8v}"
N_VIDEOS="${N_VIDEOS:-8}"
if [[ "${SMOKE:-0}" == "1" ]]; then
    N_VIDEOS=2
...
echo "---- caption pwarp preflight (first-segment, not stem) ----"
python3 - <<'PY'
...
COMMON="HORIZON_S=30,N_VIDEOS=${N_VIDEOS},SEED=0,SEARCH_FROM=0,PREFIX_LATENTS=9,CHUNK_LATENTS=21,SERIES=${SERIES},NUM_SHARDS=1,VIDEO_DIR=${VIDEO_DIR},VIDEO_WORKERS=1,PWARP_STEP=1"
...
submit_method sf_pwarp
submit_method sf_pwarp_live
...
echo "Caption pwarp N=${N_VIDEOS}. Cite vs caption Self Forcing first-8."
echo "First sidecar must print prompt_source=metadata_csv and pwarp=..."
