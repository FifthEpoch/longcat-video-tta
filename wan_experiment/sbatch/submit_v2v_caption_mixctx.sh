#!/bin/bash
# Caption N=8 mixed lock + context noise.
# Diagnostic. Not leftover ρ. Not a new timestep list.
# Hosts = caption-32 (do not remake). Do not remake cite-128.
#
...
SERIES="${SERIES:-v2v_panda_caption_mixctx_8v}"
N_VIDEOS="${N_VIDEOS:-8}"
if [[ "${SMOKE:-0}" == "1" ]]; then
    N_VIDEOS=2
...
echo "---- caption mixctx preflight (first-segment, not stem) ----"
python3 - <<'PY'
...
submit_method rf_mix
submit_method rf_mix_always
submit_method sf_mix
submit_method sf_mix_always
submit_method rolling_ctx
submit_method sf_ctx
...
echo "Caption mixctx N=${N_VIDEOS}. Cite RF arms vs Rolling first-8; SF arms vs SF first-8."
echo "First sidecar must print prompt_source=metadata_csv."
echo "ctx sidecars must have context_noise=50. mix sidecars must have mix_logs or rolled."
...
echo "    --baseline-dir ${PROJECT_ROOT}/wan_experiment/results/v2v_panda_caption_32v \\"
echo "    --series-dir ${ROOT}"
echo "Cite vs the matching host, not analyzer-vs-Self-Forcing. No TTC. No I2V. Do not remake cite-128."
