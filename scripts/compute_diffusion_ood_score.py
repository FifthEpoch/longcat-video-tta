...
# ---------------------------------------------------------------------------
# Caption parsing — Panda metadata.csv stores a stringified Python list; UCF
# entries are bare strings. Mirrors parse_caption() in extract_video_features.
# We join multi-caption clips with ". " before encoding so the text encoder
# sees the joined caption (same as the diffusion pipeline at gen time).
# ---------------------------------------------------------------------------
...
def _load_captions_csv(path: Path) -> Dict[str, str]:
    """Return {canonical_video_id -> raw caption string}."""
    out: Dict[str, str] = {}
    if not path.exists():
        print(
