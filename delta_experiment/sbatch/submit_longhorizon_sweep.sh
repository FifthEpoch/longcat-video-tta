...
echo "longer rollout, e.g. NUM_CHUNKS=18 (1440 gen frames ~90s) or 24 (1920 ~120s"
echo "= the ~2min field ceiling). Cost scales ~linearly (~9.3 min/native chunk);"
echo "drop SHARD_SIZE to 1 to stay in the 12h wall, e.g.:"
echo "  NUM_CHUNKS=18 SHARD_SIZE=1 bash delta_experiment/sbatch/submit_longhorizon_sweep.sh   # NOTTA"
echo "  NUM_CHUNKS=18 SHARD_SIZE=1 METHOD=delta_stream bash delta_experiment/sbatch/submit_longhorizon_sweep.sh"
