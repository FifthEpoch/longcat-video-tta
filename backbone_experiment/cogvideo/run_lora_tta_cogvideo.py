#!/usr/bin/env python3
"""
LoRA Test-Time Adaptation (TTA) for CogVideoX-5B-I2V.

Injects LoRA adapters into the transformer's attention layers and fine-tunes
only those parameters on conditioning frames for each video. LoRA weights
are reset between videos.

Usage:
    python run_lora_tta_cogvideo.py \
        --model-path THUDM/CogVideoX-5b-I2V \
        --data-dir /path/to/dataset \
        --output-dir results/cogvideo_lora_tta \
        --learning-rate 1e-4 --num-steps 10
"""
...
def finetune_lora_cogvideo(
    ...
    """Fine-tune LoRA parameters in CogVideoX transformer using flow-matching loss."""
...
def reset_lora_weights(transformer: nn.Module):
    """Reset LoRA adapters to their initial state (zero output)."""
    with torch.no_grad():
...
def main():
    parser = argparse.ArgumentParser(
        description="LoRA TTA for CogVideoX-5B-I2V"
    )
    ...
    print("=" * 70)
    print("LoRA TTA for CogVideoX-5B-I2V")
    print("=" * 70)
    ...
    # Update pipe's transformer reference so generation uses LoRA-wrapped model
    pipe.transformer = transformer

    # Save experiment config
    exp_config = {
        "method": "lora_tta_cogvideo",
    ...
    for idx, entry in enumerate(tqdm(videos, desc="LoRA TTA (CogVideo)")):
        if idx < start_idx:
            continue
        ...
        try:
            # Reset LoRA weights
            reset_lora_weights(transformer)
            ...
            # Fine-tune LoRA
            train_result = finetune_lora_cogvideo(
                transformer=transformer,
                latents=latents,
                prompt_embeds=prompt_embeds,
                num_steps=args.num_steps,
    ...
    print("\n" + "=" * 70)
    print("LoRA TTA for CogVideoX-5B-I2V Complete!")
    print("=" * 70)
