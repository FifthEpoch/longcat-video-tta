...
Fine-tunes ALL MMDiT parameters on conditioning frames for each video,
then generates continuations using Open-Sora's denoising pipeline.
Weights are reset between videos.

Key features:
- Unfreezes all MMDiT parameters for maximum expressivity
- Saves base state (on CPU) for per-video reset
- Uses flow-matching loss on conditioning latents
- SGD optimizer by default (no state — fits on single GPU)
...
Usage:
    python run_full_tta_opensora.py \\
        --config-path configs/diffusion/inference/256px.py \\
        --data-dir /path/to/dataset \\
        --output-dir results/full_tta_opensora \\
...
def finetune_full_opensora(
    ...
    """Fine-tune all MMDiT parameters using conditioning-aware flow-matching loss.
...
def reset_dit_weights(dit: nn.Module, base_state: Dict[str, torch.Tensor]):
    """Reset MMDiT to its pretrained weights."""
    with torch.no_grad():
...
def generate_video_opensora(
    ...
    Uses a simplified single-GPU denoising path without the distributed
    infrastructure. For v2v-style conditioning, ref_latents are masked
    into the first temporal positions.
...
def main():
    ...
    print("=" * 70)
    print("Full-Model TTA for Open-Sora v2.0")
    print("=" * 70)
    ...
    for idx, entry in enumerate(tqdm(videos, desc="Full TTA (Open-Sora)")):
        if idx < start_idx:
            continue
        ...
        try:
            ...
            # Compute num_img_tokens for text encoding alignment
            ...
            train_result = finetune_full_opensora(
                dit=dit,
                cond_latents=cond_latents,
            ...
            # Bring VAE + text encoders back for generation
            vae.to(args.device)
            t5.to(args.device)
            clip.to(args.device)
    ...
    print("\n" + "=" * 70)
    print("Full-Model TTA (Open-Sora v2.0) Complete!")
    print("=" * 70)
